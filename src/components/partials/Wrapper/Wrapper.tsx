...
import { Flex, Container, Box, useColorModeValue, ContainerProps, FlexProps } from '@chakra-ui/react';
import { MAX_WIDTH, withTokenForInnerWrapperVariant } from './tokens';
import { WrapperEnum, WrapperInnerProps } from './types';
...
export const Wrapper: React.FC<WrapperProps> = (props): JSX.Element => {
  const color = useColorModeValue('text', 'white');

  if (props.context === WrapperEnum.FLUID) {
    return (
      <Flex
        w="100%"
        alignItems="center"
  ...
  if (props.context === WrapperEnum.CONTAIN) {
    return (
      <Flex
        w={'full'}
        height={'full'}
  ...
  if (props.context === WrapperEnum.MOBILE) {
    return (
      <Container
        maxW={'2xl'}
        willChange="background, color"
  ...
  if (props.context === WrapperEnum.INNER) {
    return (
      <Flex
        w={'full'}
        height={'full'}
