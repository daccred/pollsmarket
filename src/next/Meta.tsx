...
export function openGraph({ siteName, projectTitle, description, logo = `${SITE_URL}/images/Webclip.png` }: OpenGraphType): string {
  const ogLogo = encodeURIComponent(logo);
  const ogSiteName = encodeURIComponent(siteName.trim());
  const ogProjectTitle = projectTitle ? encodeURIComponent(projectTitle.trim()) : undefined;
  ...
  return `https://og.thcl.dev/api/general?siteName=${ogSiteName}&description=${ogDesc}&logo=${ogLogo}${
    ogProjectTitle ? `&projectTitle=${ogProjectTitle}` : ''
  }`;
}
